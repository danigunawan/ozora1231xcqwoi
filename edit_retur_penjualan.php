...
$no_faktur = $_GET['no_faktur_retur'];
    ...
    $query_retur = $db->query("SELECT kode_pelanggan, tanggal, potongan, tax, ppn FROM retur_penjualan WHERE no_faktur_retur = '$no_faktur'");
    $data_retur = mysqli_fetch_array($query_retur);

    $potongan = $data_retur['potongan'];
    $tax = $data_retur['tax'];
    $ppn = $data_retur['ppn'];    
    
    $query_detail_retur = $db->query("SELECT SUM(subtotal) AS subtotal FROM detail_retur_penjualan WHERE no_faktur_retur = '$no_faktur'");
    $data_detail_retur = mysqli_fetch_array($query_detail_retur);
    $subtotal_persen = $data_detail_retur['subtotal'];
...
<!--membuat agar tabel berada dalam baris tertentu-->
 <div class="row">
          ...
          <h3> <u> FORM EDIT RETUR PENJUALAN : <?php echo $no_faktur ?> </u> </h3><br> 
  ...
  <form enctype="multipart/form-data" role="form" action="form_retur_penjualan.php" method="post ">
    ...
    <div class="row">
        ...
        <select data-placeholder="--SILAHKAN PILIH--" name="kode_pelanggan" id="kd_pelanggan" class="form-control chosen" required="" >
          <?php 
            ...
            while($data = mysqli_fetch_array($query)){
              if ($data_retur['kode_pelanggan'] == $data['kode_pelanggan']) {
      ...
      <div class="col-sm-2">
        ...
        <select type="text" name="cara_bayar" id="carabayar1" class="form-control chosen" required="" >
      ...
      <div class="col-sm-1" style="display: none">
          <input style="height: 15px" type="text" name="no_faktur_retur" id="nomorfaktur" class="form-control" readonly="" value="<?php echo $no_faktur; ?>" required="" >
      </div>

      <div class="col-sm-1">
          <label> Tanggal </label><br>
          <input style="height: 15px" type="text" value="<?php echo $data_retur['tanggal']; ?>" name="tanggal" id="tanggal" placeholder="Tanggal"  class="form-control tgl" required="" >
      </div>

      <div class="col-sm-2">
          <label> Petugas </label><br>
          <input style="height: 15px" type="text" name="user" class="form-control" readonly="" value="<?php echo $_SESSION['nama']; ?>" required="">
      </div>
...
<form action="proses_edit_tbs_retur_penjualan.php" role="form" id="formtambahproduk">
  ...
  <div class="row">
    ...
    <div class="form-group col-sm-1" >
      <input type="text" style="height: 15px" class="form-control" name="jumlah_retur" id="jumlah_retur" autocomplete="off" placeholder="0" >
    </div>
    ...
    <div class="form-group col-sm-1">
      <input type="text" style="height: 15px" id="harga_produk" name="harga" class="form-control" autocomplete="off" value="" placeholder="Harga" required="">
    </div>
    <div class="form-group col-sm-1">
      <input type="text" style="height: 15px" id="potongan1" name="potongan1" class="form-control" autocomplete="off" value="" placeholder="Disc." required="">
    </div>

    <div class="form-group col-sm-1">
      <input type="text" style="height: 15px" id="tax1" style="width: " name="tax1" class="form-control" autocomplete="off" value="" placeholder="Tax (%)" required="">
    </div>
  ...
  <div class="card card-block" style="width: 75%">
    ...
    <form action="proses_bayar_edit_retur_jual.php" id="form_beli" method="POST"><!--tag pembuka form-->
      ...
      <div class="row">
        ...
        <input type="hidden" name="tanggal" id="tanggal_hidden" class="form-control tgl" value="<?php echo $data_retur['tanggal']; ?>" >
...
$("#cari_produk_pembelian").click(function() {
  ...
  $.post("modal_edit_retur_jual_baru.php", {kode_pelanggan:kode_pelanggan, nama_toko:nama_toko, no_faktur_retur:no_faktur_retur}, function(info) {
