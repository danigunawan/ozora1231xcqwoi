...
<table id="tableuser" class="table table-bordered table-sm">
    ...
    <?php
      ...
      $perintah = $db->query("SELECT b.id AS id_produk ,dp.no_faktur, dp.tanggal, dp.kode_barang, dp.nama_barang, dp.jumlah_barang, dp.satuan AS satuan_jual, dp.harga, dp.subtotal, dp.potongan, dp.tax, dp.status, dp.sisa, p.kode_pelanggan, pl.nama_pelanggan,t.nama_toko, p.nama_konsumen, dp.asal_satuan, b.satuan AS satuan_barang , ss.nama AS satuan_real , st.nama AS satuan_asli FROM detail_penjualan dp LEFT JOIN penjualan p ON dp.no_faktur = p.no_faktur LEFT JOIN pelanggan pl ON p.kode_pelanggan = pl.kode_pelanggan LEFT JOIN barang b ON dp.kode_barang = b.kode_barang LEFT JOIN satuan ss ON dp.satuan = ss.id LEFT JOIN satuan st ON b.satuan = st.id INNER JOIN toko t ON p.kode_toko = t.id WHERE p.kode_pelanggan = '$kode_pelanggan' AND p.kode_toko = '$nama_toko' GROUP BY dp.no_faktur, dp.kode_barang");
...
if ($data['satuan_barang'] == $data['satuan_jual']) {
        ...
        // menampilkan data
      echo "<tr class='pilih' nama-konsumen ='".$data['nama_konsumen']."' data-kode='". $data['kode_barang'] ."' nama-barang='". $data['nama_barang'] ."' satuan='". $satuan_tampil ."' no_faktur='". $no_faktur_jual ."' harga='". $harga_tampil ."' jumlah-barang='". $data['jumlah_barang'] ."' sisa='". $sisa_barang_tampil ."' id_produk='". $data['id_produk'] ."'  asal_satuan ='".$data['satuan_barang']."' harga_pcs = ".$data['harga']." >
